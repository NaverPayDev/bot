...
for (const repo of NAVERPAY_REPOS) {
  if (!fs.existsSync(repo.path)) {
    console.error(
      `❌ [Embedder] 오류: 저장소 경로를 찾을 수 없습니다: ${repo.path} (저장소 이름: ${repo.name})`
    );
    process.exit(1);
  }
}
...
async function embedTextsBatch(texts) {
  try {
    ...
    console.error(
      "   [Embedder]   ⚠️ 임베딩 API 호출 중 배치 오류 발생:",
      error
    ); // 상세 오류 로깅
    return new Array(texts.length).fill(null); // API 오류 시 해당 배치의 임베딩은 null로 처리하여 건너뜀
  }
}
...
async function processCodebase() {
  ...
  for (const repo of NAVERPAY_REPOS) {
    console.log(
      `   [Embedder]   - [${repo.name}] 저장소 처리 시작 (${repo.path})`
    );
    ...
    console.log(
      `   [Embedder]     - 총 ${files.length}개의 파일/폴더 찾음. 필터링 및 내용 읽기 진행...`
    );
    ...
    for (const file of files) {
      ...
      if (RELEVANT_EXTENSIONS.includes(ext)) {
        ...
        try {
          ...
          console.warn(
            `   [Embedder]     ⚠️ 파일 처리 오류 (${path.relative(
              repo.path,
              file
            )}): ${err.message}`
          );
    ...
    console.log(
      `   [Embedder]   - [${repo.name}] 저장소에서 ${repoChunkCount}개의 유효한 코드 청크 수집 완료.`
    );
  }
  ...
  for (let i = 0; i < allCodeChunks.length; i += BATCH_SIZE) {
    ...
    vectors.forEach((vector, index) => {
      if (vector) {
        ...
        console.warn(
          `   [Embedder]     ⚠️ [${batch[index].repository}] ${batch[index].filePath} 임베딩 실패 (배치 ${currentBatchNumber}).`
        );
      }
    });

    console.log(
      `   [Embedder]   - 현재까지 성공적으로 임베딩된 수: ${embeddingsData.length}`
    );
  ...
  try {
    ...
    console.log(
      `✅ [Embedder] 완료! 임베딩 데이터가 성공적으로 '${OUTPUT_FILE}'에 저장되었습니다.`
    );
  } catch (error) {
...
processCodebase().catch((err) => {
  console.error(
    "❌ [Embedder] 스크립트 실행 중 예측하지 못한 오류가 발생했습니다:",
    err
  );
});
