...
import type {InitialPromptContext} from './prompts'
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const setApiKeyCommand = vscode.commands.registerCommand('pie-bot.setApiKey', () => {
        apiKeyManager.setApiKey(context)
    })
    context.subscriptions.push(setApiKeyCommand)

    // 3. @pie 질문 명령어 등록
    const askPieCommand = vscode.commands.registerCommand('pie-bot.askPie', async () => {
        ...
        if (pieLineNumber === -1) {
            for (let index = activeLineNumber; index >= Math.max(0, activeLineNumber - 5); index--) {
                // 현재 라인 포함 위로 5줄 탐색
                ...
                if (match && match[1]) {
                    ...
                    const targetLines = [line.text]
                    for (let i = 1; i <= TARGET_CODE_LINES_AFTER_PIE; i++) {
                        const nextLineIndex = pieLineNumber + i
        ...
        if (pieLineNumber !== -1 && userQueryText) {
            ...
            await vscode.window.withProgress(
                ...
                async (progress) => {
                    ...
                    const queryVector = await geminiApiService.embedQuery(userQueryText, apiKey)
                    if (!queryVector) {
                        return
                    }
                    ...
                    const searchResults = await ragService.searchAndRerank(queryVector, userQueryText, apiKey)

                    let ragContext = '다음 Naver Pay Dev 코드들을 참고하여 답변해주세요:\n\n'
                    if (searchResults.length > 0) {
                        for (const [index, result] of searchResults.entries()) {
                            ragContext += `--- 참고 코드 ${index + 1} (${result.repository}/${
                                result.filePath
                    ...
                    const startSurroundingLine = Math.max(0, pieLineNumber - SURROUNDING_LINES_COUNT)
                    ...
                    chatSessionManager.setSessionContext({
                        ...
                        editorSurroundingCode: surroundingCode.slice(0, 3000), // 프롬프트 길이 제한
                    })

                    const currentContextData = chatSessionManager.getCurrentSessionContext()
                    ...
                    const geminiAnswer = await geminiApiService.generateAnswer(apiKey, promptForGemini, []) // 초기 질문
