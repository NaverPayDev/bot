 ...
 */
function buildPromptTemplate(context: CombinedPromptBuilderContext, isFollowUp: boolean): string {
    const persona =
    ...
    const numbered = instructions.map((inst, index) => `${index + 1}. ${inst}`).join('\n')

    sections.push(
        '**지침**',
        numbered,
        `---\n위 지침에 따라 ${isFollowUp ? '마지막 ' : ''}사용자 질문("${context.currentUserQuery}")에 답하세요.`,
    )
