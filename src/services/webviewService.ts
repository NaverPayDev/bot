...
import {getChatHistory, addToChatHistory, getCurrentSessionContext} from './chatSessionManager'
import {generateAnswer} from './geminiApiService'
import {getFollowUpPrompt} from '../prompts'

import type {FollowUpPromptContext} from '../prompts'

let pieBotPanel: vscode.WebviewPanel | undefined
...
function getWebviewContent(webview: vscode.Webview, extensionUri: vscode.Uri): string {
    ...
    try {
        ...
        console.error('[Pie Bot] webview/chat.html 파일을 읽는 데 실패했습니다:', error)
    ...
    htmlContent = htmlContent.replaceAll('{{cspSource}}', webview.cspSource)
    htmlContent = htmlContent.replaceAll('{{styleUri}}', styleUri.toString())
...
export function createOrShowWebviewPanel(context: vscode.ExtensionContext) {
    ...
    pieBotPanel = vscode.window.createWebviewPanel('pieBotChat', 'Pie Bot Chat', vscode.ViewColumn.Beside, {
        enableScripts: true,
        localResourceRoots: [vscode.Uri.joinPath(context.extensionUri, 'webview')],
    })
    pieBotPanel.webview.html = getWebviewContent(pieBotPanel.webview, context.extensionUri)
        ...
        async (message) => {
            switch (message.type) {
                case 'userFollowUp': {
                    ...
                    const currentSessionContext = getCurrentSessionContext() // 최초 세션 컨텍스트
                    ...
                    const chatHistoryTextForPrompt = historyForAPI // 프롬프트용 문자열 이력
                        .map((entry) => `${entry.role === 'user' ? '사용자' : 'Pie Bot'}: ${entry.parts[0].text}`)
                        .join('\n\n')

                    const followUpContextData: FollowUpPromptContext = {
                        editorFilePath: currentSessionContext.editorFilePath,
                        editorSurroundingCode: currentSessionContext.editorSurroundingCode,
                        targetCodeForPrompt: currentSessionContext.targetCodeForPrompt,
                        initialQueryText: currentSessionContext.initialQueryText,
                        ragContextText: currentSessionContext.ragContextText,
